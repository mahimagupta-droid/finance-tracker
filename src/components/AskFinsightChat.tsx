...
export default function AskFinsightChat() {
    ...
    return (
        <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end">
                ...
                <div className="mb-3 flex h-[520px] w-[360px] flex-col overflow-hidden rounded-2xl border border-cyan-900/40 bg-slate-900 shadow-2xl shadow-black/40">
                    ...
                    <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto px-3 py-4">
                        ...
                        {messages.map((message) => (
                            <div
                                ...
                                <div className={`max-w-[80%] rounded-2xl px-3.5 py-2 text-sm leading-relaxed ${message.role === "user" ? "rounded-br-sm bg-cyan-500 text-slate-950" : "rounded-bl-sm bg-slate-800 text-slate-100"}`}>
                    ...
                    {error && (
                        <div className="text-left">
                            <div className="inline-block px-3 py-2 rounded bg-red-100 text-red-700">
                                <p className="text-sm">Something went wrong. Please try again.</p>
                                <button onClick={() => regenerate()} className="text-xs underline mt-1 cursor-pointer">Retry</button>
                            </div>
                        </div>
                    )}
                    <form onSubmit={handleSubmit} className="flex items-center gap-2 border-t border-cyan-900/40 bg-slate-950/60 p-3">
                        <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask FinSight anything..." className="flex-1 rounded-full border border-slate-700 bg-slate-800 px-4 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:border-cyan-500 focus:outline-none" />
            ...
            <button onClick={() => setIsOpen((v) => !v)} className="flex h-14 w-14 items-center justify-center rounded-full bg-cyan-500 text-slate-950 shadow-lg shadow-cyan-500/30 transition hover:scale-105 hover:bg-cyan-400"
