...
export default function AddTransactionsPage() {
  ...
  const [profileReady, setProfileReady] = useState<boolean | null>(null); // null = checking
  ...
  const visibleTransactions = isFiltered
    ? fetchedTransactions.filter((t) => {
      const txDate = new Date(t.date);
      return txDate >= startOfDay(dateRange.start as Date) && txDate <= endOfDay(dateRange.end as Date);
    })
    : fetchedTransactions;

  if (profileReady === null) {
    return <div className="min-h-[70vh] flex items-center justify-center text-textColor">Loading...</div>;
  }

  if (profileReady === false) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-6 gap-4">
        <h2 className="text-2xl font-semibold text-textColor">Set up your profile first</h2>
        <p className="text-muted-textColor max-w-md">
          We need a few details about you before you can start tracking transactions.
        </p>
        <button
          onClick={() => router.push("/user-profile")}
          className="bg-primary text-primary-textColor px-6 py-2 rounded-lg font-medium hover:bg-primary/85 transition"
        >
          Complete Profile
        </button>
      </div>
  ...
  return (
    <div>
      <div className="w-full max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-center px-4 sm:px-8 md:px-16 gap-6 min-h-[70vh] my-8 md:my-10">
        <div className="flex-1 flex items-center justify-center px-4 sm:px-6 w-full">
      ...
      <div className="mb-10 mt-15 bg-transparent sm:bg-card border-none sm:border border-border p-4 sm:p-5 rounded-lg transition-colors duration-300 w-full max-w-6xl mx-auto px-4 sm:px-6 md:px-10 sm:w-2/3">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            ...
            {isFiltered && (
              <button
                ...
                className="text-sm border border-border text-textColor px-3 py-1.5 rounded-lg hover:bg-border/20 transition cursor-pointer"
              >
                Clear filter
              </button>
            )}
        ...
        {!transactionsError && !loading && fetchedTransactions.length === 0 && (
          <div className="mt-6 rounded-lg bg-card p-6 text-center max-w-md mx-auto text-lg">
            No transactions added yet!
          </div>
        )}

        {!transactionsError && !loading && fetchedTransactions.length > 0 && isFiltered && visibleTransactions.length === 0 && (
          <div className="mt-6 rounded-lg border border-border bg-card p-6 text-center max-w-md mx-auto">
